...
class Kaylans(object):
    def __init__(self,ref_cat,obs_cat,imatch,\
        ...
        self.radec(ref_cat,savefig=savefig)
    ...
    def radec(self,tractor,prefix='',savefig=False): 
    ...
    def create_stack(self,answer_type,predict_type,types=['PSF','SIMP','EXP','DEV','COMP'],slim=True):
    ...
    def stacked_confusion_matrix(self,ref_tractor,test_tractor,\
                                 ref_name='ref',obs_name='test', \
        ...
        for rmin,rmax in zip(rbins[:-1],rbins[1:]):
            ...
            cm,ans_names,all_names= self.create_stack(np.array(['PSF']*len(ref_tractor)),
                                                 test_tractor.get('type'))
            cm_stack+= [cm]
        self.plot_stack(cm_stack, stack_names,all_names, \
                   ref_name=ref_name,obs_name=obs_name,prefix='',savefig=savefig)
    ...
    def delta_mag_vs_mag(self,ref_tractor,test_tractor, ref_name='ref',obs_name='test',\
                         ylim=None,prefix='',savefig=False):
        fig,ax=plt.subplots(1,3,figsize=(9,3),sharey=True)
        plt.subplots_adjust(wspace=0.25)
        ...
        ylab=ax[0].set_ylabel('mag (%s) - mag(%s)' % (obs_name,ref_name), **kwargs.ax)
        if savefig == True:
...
class Dustins(object):
    def __init__(self,ref_cat,obs_cat,imatch,d2d,\
        ...
        self.nonzero_flux_errbars(ref_cat[ imatch['ref'] ],obs_cat[ imatch['obs'] ],\
                                  ref_name=ref_name,obs_name=obs_name,savefig=savefig)
        self.nonzero_fluxdeltaflux(ref_cat[ imatch['ref'] ],obs_cat[ imatch['obs'] ],\
                                  ref_name=ref_name,obs_name=obs_name,savefig=savefig)
    ...
    def match_distance(self,dist,prefix='',savefig=False):
        ...
        if savefig == True:
            plt.savefig('%smatch_dist.png' % prefix)
            plt.close()
            
    def nonzero_flux_errbars(self,matched1,matched2,\
        ...
        if savefig == True:
            plt.savefig('%s_fluxflux_%s.png' % (band,prefix))
            plt.close()
            
    def nonzero_fluxdeltaflux(self,matched1,matched2,\
                             ref_name='ref',obs_name='obs',prefix='',savefig=False):
        fig,ax= plt.subplots(1,3)
        for cnt,iband,band,cc in [(0,1,'g','g'),(1,2,'r','r'),(2,4,'z','m')]:
        ...
        if savefig == True:
            plt.savefig('%s_fluxdeltaflux_%s.png' % (band,prefix))
            plt.close()
    ...
    def grz_ratio_fluxerr(self,matched1,matched2,\
        ...
        if savefig == True:
            plt.savefig('grz_ratio_fluxerr_%s.png' % prefix)
            plt.close()
    ...
    def magmag(self,matched1,matched2,\
        ...
        if savefig == True:
            plt.savefig('%s_magmag_%s.png' % (band,prefix))
            plt.close()
    ...
    def magdeltamag(self,matched1,matched2,\
        ...
        if savefig == True:
            plt.savefig('%s_magdeltamag_%s.png' % (band,prefix))
            plt.close()
    ...
    def deltamag_err(self,matched1,matched2,\
        ...
        for cnt,iband,band,cc in [(0,1,'g','g'),(1,2,'r','r'),(2,4,'z','m')]:
            ...
            ax[cnt].axis([24, 16, -10, 10])
        if savefig == True:
            plt.savefig('%s_deltamag_err_%s.png' % (band,prefix))
            plt.close()
    ...
    def deltamag_errbars(self,matched1,matched2,\
        ...
        if savefig == True:
            plt.savefig('%s_deltamag_errbars_%s.png' % (band,prefix))
            plt.close()
    ...
    def stephist(self,matched1,matched2,\
        ...
        if savefig == True:
            plt.savefig('%s_stephist-%s.png' % (band,prefix))
            plt.close()
    ...
    def curvehist(self,matched1,matched2,\
        ...
        for cnt,iband,band,cc in [(0,1,'g','g'),(1,2,'r','r'),(2,4,'z','m')]:
            ...
            if I_empty == False:
                ...
                ax[cnt].set_xlim(slo,shi)
        if savefig == True:
            plt.savefig('%s_curvehist_%s.png' % (band,prefix))
            plt.close()
