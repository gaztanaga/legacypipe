...
from legacyanalysis.validation.pathnames import get_indir,get_outdir,make_dir
...
def cosmos_plots(comparison='cosmos',show=False):
    indir= get_indir(comparison)
    ...
    f=open(os.path.join(get_outdir(comparison),"depth-comparisonp.txt"),"w")
    f.write("20<g<21.5 \n")
    # CREATES A FIGURE
    plt.figure(2,(5,5))
    ...
    path=os.path.join(get_outdir(comparison),"plotsRc")
    make_dir(path)
