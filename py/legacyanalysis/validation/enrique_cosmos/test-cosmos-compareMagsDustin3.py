...
def compareMags(N1 = "40", N2 = "41"):
    ...
    dat = hdu[1].data
    noJunk3 = (dat['brick_primary']) & (dat['decam_anymask'].T[1]==0) & (dat['decam_anymask'].T[2]==0) & (dat['decam_anymask'].T[4]==0) & (dat['decam_flux'].T[1]>0)  &  (dat['decam_flux'].T[2]>0) &  (dat['decam_flux'].T[4]>0) & (dat['decam_flux_ivar'].T[1]*(dat['decam_flux'].T[1])**2>25)  & (dat['decam_flux_ivar'].T[2]*(dat['decam_flux'].T[2])**2>25)  & (dat['decam_flux_ivar'].T[4]*(dat['decam_flux'].T[4])**2>25) # keep only S/N>5     
    ...
    dat = hdu[1].data
    noJunk2 = (dat['brick_primary']) & (dat['decam_anymask'].T[1]==0) & (dat['decam_anymask'].T[2]==0) & (dat['decam_anymask'].T[4]==0) & (dat['decam_flux'].T[1]>0)  &  (dat['decam_flux'].T[2]>0) &  (dat['decam_flux'].T[4]>0) & (dat['decam_flux_ivar'].T[1]*(dat['decam_flux'].T[1])**2>25)  & (dat['decam_flux_ivar'].T[2]*(dat['decam_flux'].T[2])**2>25)  & (dat['decam_flux_ivar'].T[4]*(dat['decam_flux'].T[4])**2>25) # keep only S/N>5     
    ...
    for band in bands:
        ...
        out5 = (n.median(sig2), (n.percentile(sig2,84) - n.percentile(sig2,16))/2.)
        ...
        p.xlabel("[ "+band+"("+N1+")-"+band+"("+N2+") ] / sqrt[ Var("+N2+")+Var("+N1+") ]")
        p.ylabel('Normed counts %g < %s < %g' %(maglo,band,maghi))
        p.xlim((-4,4))
        p.ylim((0,0.7))
        gp = p.legend(loc=2, fontsize=10)
        gp.set_frame_on(False)
        p.title('Cosmos '+band+N1+"-"+N2+" #= "+str(len(sig))+'(All) '+str(len(sig2)))        
        p.grid()
        p.savefig(join("plots","cosmos-comparison-depth-normed-"+band+"-v"+N1+"-"+N2+".png"))
        p.clf()
