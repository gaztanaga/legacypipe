...
if __name__ == '__main__':
    ...
    parser.add_argument("-pimages",action="store",help='full path + search string for ptf images')
    parser.add_argument("-outdir",action="store",help='full path to output dir for ccd fits table')
    args = parser.parse_args()
    
    files=glob.glob(args.pimages)
    T=exposure_metadata(files)
#save table to .fits
    T.writeto(os.path.join(args.outdir,'decals-ccds.fits'))
    print('Wrote ccd fits table')
